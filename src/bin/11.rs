...
fn expand_galaxy(text: String, fill_ch: &str) -> String {
    ...
    for i in 0..2 {
        ...
        for line in result.lines() {
            if line.chars().into_iter().all(|s| s=='.' || s==fill_ch.chars().into_iter().nth(0).unwrap()) {
...
pub fn part_one(input: &str) -> Option<u64> {
    ...
    for i in 0..galaxy_positions.len() {
        for j in i+1..galaxy_positions.len() {
            ...
            let start = Pos(galaxy_positions.get(i).unwrap().0 as i32, galaxy_positions.get(i).unwrap().1 as i32);
            let end = Pos(galaxy_positions.get(j).unwrap().0 as i32, galaxy_positions.get(j).unwrap().1 as i32);
            ...
            for path in paths {
                let mut sum_path = 0;
                for pos in path {
                    let curr_char = get_char_at_position(&file_content2, pos.0 as usize, pos.1 as usize).unwrap();
                    if curr_char == 'X' {
                        sum_path += 2
                    } else {
                        sum_path += 1
