        ...
        for (point, direction) in last_points {
            ...
            if let Some(curr_ch) = get_char_at_point(input, &point) {
                ...
                for next_point in next_points2.clone().iter() {
                    if light_points
                        .iter()
                        .any(|p| p.0 == next_point.0 && p.1 == next_point.1)
                    {
...
fn calculate_next_points(
    row_count: usize,
    col_count: usize,
    next_points: &mut Vec<(Point, Direction)>,
    point: &Point,
    direction: Direction,
    curr_ch: char,
) {
    match curr_ch {
        ...
        _ => {
            panic!("Unknown character {}", curr_ch);
        }
    }
}
        ...
        for j in 0..row_count {
            let mut start_points = if i == 0 && j == 0 {
                vec![
                    ...
                    (Point { row: 0, col: 0 }, Direction::DOWN),
                ]
                    ...
                    for (point, direction) in last_points {
                        ...
                        if let Some(curr_ch) = get_char_at_point(input, &point) {
                            ...
                            for next_point in next_points2.clone().iter() {
                                ...
                                {
                                    next_points2
                                        .retain(|p| p.0 != next_point.0 && p.1 != next_point.1);
                                } else {
                                    light_points.push((*next_point.0, *next_point.1));
                                }
                            }
